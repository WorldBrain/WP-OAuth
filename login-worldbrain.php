...
$_SESSION['WPOA']['PROVIDER'] = 'WorldBrain';
define('HTTP_UTIL', 'stream-context');
define('CLIENT_ENABLED', get_option('wpoa_worldbrain_api_enabled'));
define('CLIENT_ID', get_option('wpoa_worldbrain_api_id'));
define('CLIENT_SECRET', get_option('wpoa_worldbrain_api_secret'));
define('REDIRECT_URI', rtrim(site_url(), '/') . '/');
define('SCOPE', 'profile'); // PROVIDER SPECIFIC: 'profile' is the minimum scope required to get the user's id from Google
define('URL_AUTH', "https://memex.cloud/oauth/start?");
define('URL_TOKEN', "https://memex.cloud/oauth/token?");
define('URL_USER', "https://memex.cloud/oauth/profile?");
define('URL_USER_LINK', "https://memex.cloud/oauth/wp-link?");
...
elseif (isset($_GET['code'])) {
	if ($_SESSION['WPOA']['STATE'] == $_GET['state']) {
		...
		$this->post_link_user = 'send_user_id_to_worldbrain';
...
function send_user_id_to_worldbrain($user_id) {
	$url = rtrim(URL_USER_LINK, "?");
	file_put_contents(__DIR__ . "/log.txt", "sending profile info to $url", FILE_APPEND);
